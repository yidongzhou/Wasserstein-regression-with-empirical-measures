...
ise <- foreach(n = nVec, .combine = 'cbind') %:%
  foreach(icount(Q), .combine = 'c', .options.snow = list(progress = progress)) %dopar% {
    ...
    while (length(N) < n) {
      ...
      if (N[length(N)] < 2) {# == 0 enough for lrem. Petersen and Müller (2019) won't work if N_i=1
        N <- N[-length(N)]
      }
    }
    ...
    for(i in 1:n) {
      ...
      yMean[i, ] <- eta0 + alpha * sin(pi * x[i]) + (sigma0 + beta * sin(pi * x[i])) * qnorm(1:(M - 1) / M)
...
save(isel0, isel, iselp, isel0T, iselT, iselpT, file = 'data/isel.RData')
stopCluster(cl)
